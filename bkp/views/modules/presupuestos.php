...
<body>
  ...
  <?php
      ...
      include 'views/modules/modals/presupuestos.php';
...
<script type="text/javascript">
        $(document).ready(function(){
            ...
            {
                $("#formulario")[0].reset();
                $(".modal-title").text("Crear Presupuesto");
                $("#action").val("Crear");
                $("#operacion").val("Crear");
		    ...
		    if(nombres != '' && codigo != '' )
                {
                    $.ajax({
                    url: base_url + "/assets/ajax/presupuestos.php?op=guardar&cliente=" + cliente_id,
                    method: 'POST',
            ...
            $.ajax({
                url:"/assets/ajax/presupuestos.php?op=buscar",
                method:"POST",
                ...
                success:function(data)
                    {
                        ...
                        $('.modal-title').text("Editar Presupuesto");
            ...
            $(document).on('click', '.borrar', function(){
                ...
                {
                    $.ajax({
                        url:base_url+"/assets/ajax/presupuestos.php?op=eliminar",
                        method:"POST",
                        data:{id_usuario:id_usuario},
                        success:function(data)
                        {
            ...
            //dataTable
             ...
             var dataTable = $('#tblpresupuestos').DataTable({
                ...
                "order":[],
                "ajax":{
                    url: base_url+"/assets/ajax/presupuestos.php?op=presupuestos",
                    type: "POST"
                },
                "columnsDefs":[
                    {
    ...
    // ======== CARGAR CLIENTES ===========
$.ajax({
    url: base_url + "/assets/ajax/presupuestos.php?op=clientes",
    type: "POST",
    dataType: "json",
    success: function(data) {
...
$('#clientes').on('change', function() {
    ...
    dataTable.ajax.url(base_url + "/assets/ajax/presupuestos.php?op=presupuestos&cliente=" + cliente_id).load();
});
