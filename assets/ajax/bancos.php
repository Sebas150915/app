...
switch ($variable) 
{
    ...
    $stmt_total = $connect->prepare("SELECT COUNT(*) AS total FROM tbl_bancos WHERE cliente = :idempresa");
    ...
    $query = "SELECT * FROM tbl_bancos WHERE cliente = :idempresa ";
    ...
    foreach ($resultado as $fila) {
        ...
        $sub_array[] = htmlspecialchars($fila["codigo"]);
         $sub_array[] = htmlspecialchars($fila["origen"]);
         $sub_array[] = htmlspecialchars($fila["moneda"]);
    ...
    {
        $stmt = $connect->prepare("INSERT INTO tbl_bancos(cliente, nombre, codigo,origen,moneda) VALUES(:cliente, :nombre, :codigo, :origen,:moneda)");
    ...
    else
    {
        ...
        $stmt = $connect->prepare("UPDATE tbl_bancos SET nombre=:nombre, codigo=:codigo, origen=:origen, moneda = :moneda WHERE id = :id");
    ...
    $salida = array();
    $stmt = $connect->prepare("SELECT * FROM tbl_bancos WHERE id = '".$_POST["id_usuario"]."' LIMIT 1");
    $stmt->execute();
    ...
    $stmt = $connect->prepare("SELECT estado FROM tbl_bancos WHERE id = :id LIMIT 1");
    ...
    if ($fila) {
        ...
        $stmt = $connect->prepare("UPDATE tbl_bancos SET estado = :estado WHERE id = :id");
        $resultado = $stmt->execute([
            ':estado' => $estado,
            ':id' => $_POST["id_usuario"]
        ]);
