...
switch ($variable) {
    ...
    case 'listar_cc':
        ...
        if (!$cliente_id) {
            echo json_encode(['error' => 'No se ha definido el cliente']);
            exit;
        }
        $query = "SELECT * FROM tbl_centro_costo WHERE cliente = :cliente_id ORDER BY id ASC";
        $stmt = $connect->prepare($query);
    ...
    case 'listar_pre':
        ...
        $query = "SELECT * FROM tbl_presupuestos WHERE cliente = :cliente_id ORDER BY id ASC";
        $stmt = $connect->prepare($query);
        $stmt->bindValue(':cliente_id', $cliente_id, PDO::PARAM_INT);
        $stmt->execute();
        echo json_encode($stmt->fetchAll(PDO::FETCH_ASSOC));
    ...
    case 'centro_costos':
        ...
        $stmt_total = $connect->prepare("SELECT COUNT(*) AS total FROM tbl_centro_costo WHERE cliente = :idempresa");
        ...
        $query = "SELECT * FROM tbl_centro_costo WHERE cliente = :idempresa ";
        ...
        foreach ($resultado as $fila) {
            ...
            $sub_array[] = htmlspecialchars($fila["codigo"]);
    ...
    case 'guardar':
        ...
        if ($_POST['operacion'] == 'Crear') {
            $stmt = $connect->prepare("INSERT INTO tbl_centro_costo(cliente, nombre, codigo) VALUES(:cliente, :nombre, :codigo)");
            ...
            $stmt = $connect->prepare("UPDATE tbl_centro_costo SET nombre=:nombre, codigo=:codigo WHERE id = :id");
    ...
    case 'buscar':
        ...
        $salida = array();
        $stmt = $connect->prepare("SELECT * FROM tbl_centro_costo WHERE id = '" . $_POST["id_usuario"] . "' LIMIT 1");
        $stmt->execute();
    ...
    case 'eliminar':
        ...
        $stmt = $connect->prepare("SELECT estado FROM tbl_centro_costo WHERE id = :id LIMIT 1");
        ...
        if ($fila) {
            ...
            $stmt = $connect->prepare("UPDATE tbl_centro_costo SET estado = :estado WHERE id = :id");
            $resultado = $stmt->execute([
                ':estado' => $estado,
                ':id' => $_POST["id_usuario"]
            ]);
